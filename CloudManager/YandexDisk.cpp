	...
	: AbstractCloud(qsettingsGroup)
{
	...
	//connect(&disk, &QNetworkAccessManager::finished, this, &QAbstractManager::setReplyFinished);	// сигнал посылаетс€, когда получен ответ сервера, но это не значит, что закончилась обработка ответа
	//есть смысл делать всЄ асинхронно
...
Request YandexDiskWebDav::downloadFile(const ShortName& name, QSharedPointer<QIODevice> file)
{
	...
	connect(reply, &QNetworkReply::finished, this, [=](){
		...
		if (!ok) qDebug() << QString::fromLocal8Bit("’уйн€");
		reply->deleteLater();
		//emit done();
	});
